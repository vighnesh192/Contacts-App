...
function Contacts() {
    ...
    const handleSubmitClick = (e) => {
        e.preventDefault()
        try {
            axios.post('http://localhost:3001/contact', { name, number, email }, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } })
                .then((res) => {
    ...
    function getContacts() {
        axios.get('http://localhost:3001/contact', { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } })
            .then((res) => {
